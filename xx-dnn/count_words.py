...
class WordTable(object):
    ...
    def encode_one_hot(self, W, forConv=False):
        ...
        if type(W) is string:
            ...
            x = (
                np.zeros((SAMPLE_SIZE, VOCAB_SIZE))
                if not forConv
                else np.zeros((SAMPLE_SIZE, VOCAB_SIZE, 1))
            )
...
ctable = WordTable()
print(
    f"Words table with training size {TRAINING_SIZE}, batch size {BATCH_SIZE}, vocab size {VOCAB_SIZE} and sample size {SAMPLE_SIZE}"
)
...
def input_generator(nsamples, train=True, forConv=False):
    ...
    with open(f_x) as fx, open(f_y) as fy:
        j = 0
        x = (
            np.zeros((nsamples, SAMPLE_SIZE, VOCAB_SIZE), dtype=np.int)
            if not forConv
            else np.zeros((nsamples, SAMPLE_SIZE, VOCAB_SIZE, 1), dtype=np.int)
        )
...
def model_ff():
    ...
    model.compile(loss="binary_crossentropy", optimizer="adam", metrics=["acc", topcats])
...
def model_convnet1D():
    ...
    epochs = 1
    model = Sequential()
    model.add(layers.Conv1D(32, 10, activation="relu", input_shape=(SAMPLE_SIZE, VOCAB_SIZE)))
...
def model_convnet2D():
    ...
    epochs = 150
    model = Sequential()
    model.add(layers.Conv2D(VOCAB_SIZE, (1, VOCAB_SIZE), input_shape=(SAMPLE_SIZE, VOCAB_SIZE, 1)))
    model.add(layers.Lambda(SumPooling2D))
    model.add(layers.Reshape((VOCAB_SIZE,)))
    #    model.add(layers.Flatten())
    #    model.add(layers.Dense(VOCAB_SIZE))

    model.compile(loss="mean_squared_error", optimizer="adam", metrics=["acc", topcats])
